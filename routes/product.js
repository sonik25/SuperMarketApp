...
router.get('/category/:id/page/:pageIndex', async(req,res) =>{
    try{
        ...
        let pageSize = Math.ceil(prodCount / perPage);
        if(!prodList){
                return res.status(403).send({message:"Invalid product details"})
            }
        //res.send(prodList);
        res.send({
            perPage: perPage,
...
router.get('/category/:id/subcategory/:subid', async(req,res) =>{
    try{
        ...
        if(!subcat){ res.status(403).send({message:"Sub Category not found"})}

        let categoryList = await Cate.Category.findById(req.params.id);
        console.log(categoryList.name);
        if(!categoryList){ return res.status(403).send({message:"Category not found"})  }       

        let prodList = await Prod.Product.find({'category': categoryList.name,'subCategory':subcat.name});
        if(!prodList){ return res.status(403).send({message:"Invalid product details"}) }
...
router.get('/offerProduct',async(req,res) =>{
    try{
        ...
        if(!result) { res.status(401).send({message:'No Product in offer'})}
        res.send(result);
